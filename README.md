...
- GUI 证据预览支持聚焦两侧匹配区域和一键复制证据摘要；
- Windows/Linux GitHub CI、静态检查和自动化测试。
- `pyside6-deploy`/Nuitka Windows standalone 免安装包工作流。

当前整体感知、局部几何、Western blot、荧光、病理、Excel 近似和关系结果均需人工复核。Western blot 复杂多面板拆分、任意区域擦除/拼接，荧光实验组语义，病理连续切片语义，通用单图 Copy-Move，Excel 自动识别/手动框选扫描、历史库和可恢复任务断点仍在后续里程碑；统计相似不会自动判定重复。
