...
当前在 `codex/wechat-paper-dataset-feasibility` 完成首篇 Figcheck 流程试点，以及后续 42 个公众号案例的全文主图获取、候选分流和交互复核包。真实公众号正文、标注图、论文 Figure、附件响应、清单、交互包和反馈只保存在 Git 忽略目录；该方向尚未进入 PRD 或正式开发。
...
- 本地开发版本为 `0.1.0a15`，扫描算法版本为 `generic-image-local-2+western-blot-2+dot-blot-2+fluorescence-1+pathology-2+excel-advanced-4`。
...
5. 冻结 `generic-image-local-2` 和当前 34 条开发回归，不再围绕同集继续调阈值；收集按 `source_group` 隔离的新论文作为独立 test。22 个未恢复彩框关系组继续由系统处理，13 条 `insufficient_evidence` 暂不计分。
...
- `pip check`：通过。
- `pytest`：129 项通过；新增并保留 Dot blot 回归，以及 `generic-image-local-2` 的白底图表抑制、小区域密集结构/SIFT、无关小条带压力和 GUI 证据解释测试。
- `git diff --check`：通过。
