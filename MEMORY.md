...
- 本地开发版本为 `0.1.0a17`，扫描算法版本为 `generic-image-local-2+western-blot-2+dot-blot-3+fluorescence-1+pathology-2+panel-split-1+excel-advanced-4`。
...
- `generic-image-local-2` 按 Western blot、病理、荧光和通用图片路由后，当前 33 条开发回归为 25 TP、0 FP、0 FN、8 TN。自动模式由斑点排列检测器命中的非 Dot blot 结构只对用户显示“局部重复结构”，不宣称实验类型；显式 Dot blot 模式仍保留专项解释。该批样本已参与调参，只能作为 validation，不能把同集满分称为独立准确率。
...
3. 冻结当前 33 条开发回归和现有阈值，不再围绕同集调参；补足来源隔离的 Western、HE/IHC 和免疫荧光正负例，建立未参与调参的独立 test 后再报告准确率。
