...
当前在 `codex/auto-dot-routing` 完成 AUTO Dot blot 路由和大批量候选预算优化，并将算法升级为 `generic-image-local-3+western-blot-2+dot-blot-4+fluorescence-1+pathology-2+panel-split-1+excel-advanced-4`。42 个公众号案例的数据闭环和可选复合图拆分已在前续阶段完成。真实公众号正文、标注图、论文 Figure、附件响应、清单、交互包、反馈和本地结果只保存在 Git 忽略目录。
...
- 本地开发版本为 `0.1.0a18`，扫描算法版本为 `generic-image-local-3+western-blot-2+dot-blot-4+fluorescence-1+pathology-2+panel-split-1+excel-advanced-4`。
...
- 新收集的 124 例已用于本次路由/超时诊断和优化，因此已降级为 validation/regression，不能再作为来源独立的 final test。固定 120 秒门限的整图/Panel 双模式重跑已完成：248 次扫描均无超时或错误；Panel 对 35 个可评阳性案例均给出候选，但 61 个可评阴性案例中 59 个也有待复核候选。没有逐关系 Figure/Panel 区域真值，precision、F1 和区域级召回均为 `N/A`，尚未达到上线验收条件。
...
3. 冻结当前 33 条和 124 例开发回归；先让医学人员复核 124 例轻量包中的代表性候选，主要确认默认阴性中的误报类型和公开来源映射，再依据复核结果降低通用局部误报。随后另收集来源隔离的 Western、HE/IHC 和免疫荧光正负例，建立未参与调参的独立 test 后再报告准确率和决定是否上线。
