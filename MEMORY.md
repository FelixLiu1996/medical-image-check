...
当前在 `codex/composite-panel-splitting` 开发可选复合图自动拆分、扫描前子面板预览/勾选和原图证据坐标回映。真实图片、裁剪、清单和结果只保存在 Git 忽略目录。
...
- 本地开发版本为 `0.1.0a16`，扫描算法版本为 `generic-image-local-1+western-blot-2+dot-blot-2+fluorescence-1+pathology-2+panel-split-1+excel-advanced-4`。
...
1. 完成 `codex/composite-panel-splitting` 全量本地验证并推送，后台监控最新 CI 和 Windows portable；通过后等待用户确认是否合入 `main`。
2. 使用新一批复合 WB/HE 图片做本地只读回归，记录欠拆、过拆和跨图小区域召回，不把真实样例提交公有仓库。
3. 补足来源隔离的 Western、HE/IHC 和免疫荧光正负例；建立未参与调参的 test 集后再报告准确率。
