...
- 当前版本为 `0.1.0a12`，扫描算法仍为 `generic-image-local-1+western-blot-1+dot-blot-1+fluorescence-1+pathology-2+excel-advanced-3`；当前计算后端仍固定为 CPU，尚未启用 CUDA。
- 最新 Excel 阶段 Windows/Linux CI 和 Windows portable 已通过；打包程序三报告冒烟、许可证收集、ZIP 组装和工件上传均成功。

## 下一步

1. 推送 `codex/gpu-performance-profiling` 并确认 Windows/Linux CI 与 Windows portable。
2. 在 RTX 3080 Ti 参考机上运行图片和 Excel 代表性任务，导出性能诊断并与 CPU 基线对照。
3. 根据耗时占比选择首批 GPU 加速阶段，再单独确认 CUDA 后端、发行依赖、CPU/GPU 一致性和回退验收。
4. 获取独立正负例验收数据后继续校准算法阈值、候选上限和误报分层。
...
- Ruff 检查与格式检查通过
- `pip check` 通过
- pytest 110 项通过；新增阶段计时与暂停扣除、NVIDIA/OpenCV CUDA 探测及失败隔离、性能画像项目迁移、诊断隐私和 GUI 导出测试
- Qt offscreen 启动通过
