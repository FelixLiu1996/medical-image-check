...
- 当前开发版本为 `0.1.0a16`，扫描算法为 `generic-image-local-2+western-blot-2+dot-blot-2+fluorescence-1+pathology-2+panel-split-1+excel-advanced-4`；当前计算后端仍固定为 CPU，尚未启用 CUDA。
...
- 两轮反馈已收口为 34 条 validation：25 positive、9 negative；另保留 4 条无效、去重或不适合正式主图训练的排除记录。当前完整基线为 13 TP、5 FP、12 FN、4 TN，precision 0.722、recall 0.520、specificity 0.444。该批数据已参与流程与算法诊断，只能用于开发回归，不能作为独立 test 或对外准确率。
- `generic-image-local-2` 已针对这 34 条中的白底统计图/组合版式误报和小区域漏检增加布局抑制、有界密集结构及 SIFT 仿射复核；同集开发回归为 25 TP、0 FP、0 FN、9 TN。48 张独立生成的无关灰度小条带形成 1,128 个可能图片对，输出 0 条小区域结果，端到端约 4.5 秒。34 条本身参与了调参，现已冻结为回归集；上述数字不能解释为独立准确率或泛化效果。
...
5. 冻结 `generic-image-local-2` 和当前 34 条开发回归，不再围绕同集继续调阈值；优先收集按 `source_group` 隔离的新论文建立独立 test。22 个尚未恢复的彩框关系组继续由系统处理，13 条 `insufficient_evidence` 在取得许可明确的正式主图前不计分。
...
- 当前通用图像阈值已通过合成数据和 34 条同集开发回归，但尚未通过来源隔离的独立 test，不能作为准确率承诺。
...
- 公众号论文样本链路已完成 42 个案例的正文资料、64 篇论文身份、306 张正式主图、27 条来源图片对、8 条程序候选、两轮医学反馈和 34 条 validation 收口。当前阻塞转为 22 个未恢复彩框关系组的系统映射、13 个缺正式主图案例的合规资产补全，以及用于验证 `generic-image-local-2` 泛化效果的来源隔离新 test；当前同集回归不能作为独立准确率。真实正文、图片、交互包和反馈均未提交仓库。
...
- `panel-split-1` 与 `generic-image-local-2` 合入后，macOS ARM64/Python 3.12.13 下 `ruff format --check`、`ruff check`、`pip check` 和 `git diff --check` 通过，`QT_QPA_PLATFORM=offscreen` 下 pytest 138 项全部通过；拆分保持默认关闭，34 条开发回归仍为 25 TP、0 FP、0 FN、9 TN。
