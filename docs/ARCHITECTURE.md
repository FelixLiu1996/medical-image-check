...
`medical_image_check.evaluation.dot_blot` 是开发期本地评测入口：读取外部 JSON 路径/逐对标签清单，复用显式 Dot blot 扫描模式并输出分类指标。评测清单与真实图片不进入项目文件、GUI 或发行包的数据资产，也不会反向修改扫描器；同一来源组跨数据集会被拒绝，详细格式见 `docs/DOT_BLOT_EVALUATION.md`。
