...
- `Project` 使用 `schema_version = 3`，包含 UUID、名称、时间、源路径、连续数字片段最短位数、最近一次扫描结果和报告路径。
...
- 版本 1/2 项目清单会在内存中迁移为版本 3，缺失的结果、报告和片段设置使用空值或默认 4 位；下一次保存写为版本 3。
- `Finding.details` 支持递归 JSON 证据，Excel 片段/近似结果保存逐单元格完整值，列关系保存逐位置配对值与运算结果。

输入路径或连续数字片段最短位数变化会使最近扫描结果失效，防止导出与当前输入/参数不一致的旧结果。当前 `.mic-project.json` 仍是开发阶段清单格式，不等同于 PRD 中最终的可移植项目包，也尚未缓存图片特征。
