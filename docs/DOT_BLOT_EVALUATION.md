...
评测器只读取清单和原图片，调用显式 Dot blot 模式，输出 TP、FP、FN、TN、precision、recall、specificity、逐对证据和读取问题。它不会复制图片、联网、训练模型或修改算法阈值。Windows 将解释器路径替换为 `.venv\\Scripts\\python.exe`。
...
4. 报告总体指标的同时保留逐来源组指标和失败案例；同一来源产生的多个增强图不能当作多个独立真实样本宣传。
5. 每次修改阈值都更新 `docs/ALGORITHMS.md`、`docs/TESTING.md` 和变更日志，并保留可复现的合成回归。
