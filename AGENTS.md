...
本文件是 Medical Image Check 仓库的开发与协作约定。
...
- 所有代码、测试和文档修改都必须在开发分支上进行，禁止直接在 `main` 分支开发或提交。开始修改前，先从最新的 `main` 创建开发分支。
...
开发分支推送后，如果触发远程 CI 或打包：

- 记录工作流名称、run ID 和链接，并在不阻断主对话的情况下后台监控到终态。
- 成功时简要记录结果；失败时必须反馈失败的工作流、Job/步骤、关键错误、原因判断和建议修复方案，不能只报告“构建失败”。
- 构建仍在运行时可以继续处理不依赖该结果的工作；合并、打标签或发布前必须重新确认所有必需工作流已经成功。
- 后续提交触发新 run 时，以最新提交对应的 run 为准，不得把旧 run 的成功误认为当前分支已经通过。
